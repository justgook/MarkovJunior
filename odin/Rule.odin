...
rule_init :: proc(g: ^Grid, in_string, out_string: string) -> Rule {
	...
	return rule_from_arrays(g, input, pin.mx, pin.my, pin.mz, output, pout.mx, pout.my, pout.mz)
}

rule_from_arrays :: proc(g: ^Grid, input: []i32, imx, imy, imz: int, output: []u8, omx, omy, omz: int) -> Rule {
	r := Rule{
...
rule_clone :: proc(g: ^Grid, r: ^Rule) -> Rule {
	...
	return rule_from_arrays(g, input, r.imx, r.imy, r.imz, output, r.omx, r.omy, r.omz)
}
...
rule_z_rotated :: proc(g: ^Grid, r: ^Rule) -> Rule {
	...
	return rule_from_arrays(g, input, r.imy, r.imx, r.imz, output, r.omy, r.omx, r.omz)
}
...
append_square_symmetries :: proc(g: ^Grid, rules: ^[dynamic]Rule, base: Rule) {
	things: [8]Rule
	things[0] = base
