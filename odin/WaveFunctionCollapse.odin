...
wfc_go :: proc(w: ^WFC_State, g: ^Grid, random: ^MJRandom) -> bool {
	...
	if w.firstgo {
		...
		if w.tile_mode {
			...
			// interpreter output to the expanded tile grid immediately. The Odin port
			// keeps `g` coarse for WFC computation and swaps `newgrid` in for output
			// while observation is still in progress.
			return true
		}
		for i in 0..<len(w.newgrid.state) do w.newgrid.state[i] = 0
